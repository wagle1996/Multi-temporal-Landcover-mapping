...
for (ntree in c(500,1000, 1500, 2000, 2500)) {
  set.seed(100)
  fit <- train(Class~B4+B2+B3+B6+B5+B7+NDVI+NDBI+MNDWI+elevation+Slope, data=test_split, method="rf", metric='Accuracy', tuneGrid=tunegrid, trControl=control, ntree=ntree)
  key <- toString(ntree)
  modellist[[key]] <- fit
}
...
mytxt_x <- file.path(paste0(getwd(),"/Results/", "LS8_2020_CM_xgb.txt"))
sink(mytxt_x)
...
# Predict!
library(caret)
LS8_2020_pred_xgb <- predict(LS8_2020, model=xgb_model, na.rm=T)
plot(LS8_2020_pred_xgb)
...
# 
# mytxt_x <- file.path(paste0(getwd(),"/Results/", "LS8_2020_CM_rpart.txt"))
# sink(mytxt_x)
...
# # Predict!
# library(caret)
# LS8_2020_pred_rpart <- predict(LS8_2020, model=rpart_model, na.rm=T)
# plot(LS8_2020_pred_rpart)
# 
# # write to a new geotiff file
