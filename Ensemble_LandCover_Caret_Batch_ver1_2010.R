...
# model_rfe <- rfe(training_image_2010[,2:12],training_image_2010[,13],sizes=c(2:12),rfeControl=trainctrl_rfe)
...
for (i in list_model) {
  ...
  mytxt <- file.path(paste0(getwd(),"/BatchResults/","LS8_2010_CM_",i,".txt"))
  sink(mytxt)
  ...
  plot(LS8_2010_pred,main=i,col=colorRampPalette(c("red", "blue","green","grey","yellow"))(255))
  mytif <- file.path(paste0(getwd(),"/BatchResults/","LS8_2010_pred_",i,".tif"))
...
#Estimate time
mytxt <- file.path(paste0(getwd(),"/BatchResults/","LS8_2010_Consumedtime.txt"))
sink(mytxt)
